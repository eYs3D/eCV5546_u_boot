...
#elif defined(CONFIG_SYS_ENV_eCV5546_EVB)
#define CONFIG_SYS_SDRAM_SIZE           (1024 << 20)
#else /* normal eCV5546 evb environment can have larger DRAM size */
#define CONFIG_SYS_SDRAM_SIZE           (1024 << 20)
#endif
...
#include <asm/arch/sp_bootmode_bitmap_ecv5546.h>
 ...
 * But now sp_go DO verify magic/hcrc/dcrc in the quick eys3d uIamge header.
...
"loadbootenv=fatload ${isp_if} ${isp_dev}:1 ${scriptaddr} /${bootenv} || " \
	...
	"fatload ${isp_if} ${isp_dev}:1 ${scriptaddr} /eys3d/ecv5546/${bootenv}; " \
	"\0" \
...
#if defined(CONFIG_VIDEO_eCV5546) && !defined(CONFIG_DM_VIDEO_eCV5546_LOGO)
...
#if defined(CONFIG_USB_KEYBOARD) && !defined(CONFIG_DM_VIDEO_eCV5546_LOGO)
