/* AUTOGENERATED FILE - DO NOT EDIT! */
/* This file is provided by the meta-mender layer. */

#ifndef HEADER_CONFIG_MENDER_DEFINES_H
#define HEADER_CONFIG_MENDER_DEFINES_H

#ifdef CONFIG_MENDER_OTA_INTEGRATION

/* Shell variables */
#define MENDER_BOOT_PART_NUMBER 1
#define MENDER_BOOT_PART_NUMBER_HEX 1
#define MENDER_ROOTFS_PART_A_NUMBER 10
#define MENDER_ROOTFS_PART_A_NUMBER_HEX a
#define MENDER_ROOTFS_PART_B_NUMBER 11
#define MENDER_ROOTFS_PART_B_NUMBER_HEX b
#define MENDER_UBOOT_STORAGE_INTERFACE "mmc"
#define MENDER_UBOOT_STORAGE_DEVICE 0

/* BB variables. */
#define MENDER_STORAGE_DEVICE_BASE "/dev/mmcblk0p"
#define MENDER_UBOOT_ENV_STORAGE_DEVICE_OFFSET_1 0x304400
#define MENDER_UBOOT_ENV_STORAGE_DEVICE_OFFSET_2 0x384400
#define MENDER_ROOTFS_PART_A_NAME "/dev/mmcblk0p10"
#define MENDER_ROOTFS_PART_B_NAME "/dev/mmcblk0p11"

/* For sanity checks. */
#define MENDER_BOOTENV_SIZE 0x20000

#define MENDER_BOOT_KERNEL_TYPE "booti"
#define MENDER_KERNEL_NAME "uImage"
#define MENDER_DTB_NAME "dtb"
#define MENDER_UBOOT_PRE_SETUP_COMMANDS ""
#define MENDER_UBOOT_POST_SETUP_COMMANDS ""

#endif /* CONFIG_MENDER_OTA_INTEGRATION */

#endif /* !HEADER_CONFIG_MENDER_DEFINES_H */
