/*
 * This is a driver for the SDHC controller found in eYs3D Gemini SoC
 *
...
static int sp_mmc_probe(struct udevice *dev)
{
	...
	ret = clk_enable(&clk);
	if (ret < 0) {
		IFPRINTK("failed to enable eys3d SD clk\n");
		return ret;
	}
	...
	IFPRINTK("version type: %s\n", (host->dev_info.version == SP_MMC_VER_eCV5546) ? "eCV5546" : "Q645");
...
static sp_mmc_dev_info sp_dev_info[] = {
	{
		...
		.version = SP_MMC_VER_eCV5546,
	},
};


static const struct udevice_id eys3d_mmc_ids[] = {
	{
...
U_BOOT_DRIVER(sdcard_eys3d) ={
	.name		= "sd_eys3d",
	.id		= UCLASS_MMC,
	.of_match	= eys3d_mmc_ids,
