 ...
 **************************************************************************/
void DRV_mipitx_gpio_set(struct ecv5546_disp_priv *sp)
{
	/* reset panel */
	#if 0 //active low
...
void DRV_mipitx_pllclk_set(int mode, int width, int height)
{
	if (mode == 0) { //LP mode
		...
		#if defined(eCV5546_MIPI_DSI_1500M_TEST) //case in TXPLL = 1500M, then PLLH will be 250MHz
		...
		#elif defined(eCV5546_MIPI_DSI_1200M_TEST) //case in TXPLL = 1200M, then PLLH will be 200MHz
			DISP_MOON3_REG->sft_cfg[14] = 0x00780058; //PLLH
			DISP_MOON3_REG->sft_cfg[14] = (0x7f800000 | (0x10 << 7)); //PLLH
...
void DRV_mipitx_panel_Init(int is_mipi_dsi_tx, int width, int height)
{
	if (is_mipi_dsi_tx) {
		if ((width == 480) && (height == 1280)) {
			...
			DRV_mipitx_gpio_set(( struct ecv5546_disp_priv *)sp_gpio);
